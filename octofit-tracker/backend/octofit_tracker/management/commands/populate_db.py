...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        app_models.Activity.objects.create(user=superman, type='Bike', duration=60, calories=600)
        app_models.Activity.objects.create(user=captain, type='Yoga', duration=20, calories=100)

        # Create Workouts
        app_models.Workout.objects.create(name='Morning Cardio', description='Cardio for all heroes', duration=40)
        app_models.Workout.objects.create(name='Strength Training', description='Strength for all heroes', duration=50)

        # Create Leaderboard
        app_models.Leaderboard.objects.create(team=marvel, points=400)
        app_models.Leaderboard.objects.create(team=dc, points=500)
